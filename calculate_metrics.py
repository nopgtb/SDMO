...
def load_rfm_commits(repository):
    rfm_commits = []
    if Util.file_exists(repository["commit_messages"]):
        ...
        for repository in repositories:
            ...
            if not Util.file_exists(metric_report):
                ...
                if repository_rfm_metrics:
                    ...
                    repository["metric_report"] = "METRIC_ERROR"
                    print(Util.get_timestamp(), " Could not fetch metrics for ",  repository["source_git"])
            else:
