...
class Metric_RFC(Metric_Interface):
    ...
    def get_metric(self, commit_hash):
        ...
        if metric_data and commit_hash in metric_data.keys():
            #for class in metric_data[hash] {class["class"] : class["rfc"]}
            return [{"class":c["class"], "metric":c["rfc"]} for c in metric_data[commit_hash]]
        return None
